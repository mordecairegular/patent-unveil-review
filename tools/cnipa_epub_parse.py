# -*- coding: utf-8 -*-
"""
解析 http://epub.cnipa.gov.cn/ 检索结果页 HTML，提取公布公告列表中的标题、公开号、提示链接、摘要（若有）。

注意：公布公告结果页中的二维码 title 或 /patent/{公开号} 路径不能视为稳定可复核详情页。
本模块只做结果页候选解析，不做详情页打开验证；因此默认不把这类 URL 写入 `link`，
而是写入 `cnipa_qr_or_hint_url`。
...
class EpubSearchHit:
    ...
    # 仅用于已实际打开并核验过的稳定公开 URL；本解析器不会自行填充。
    link: str | None = None
    cnipa_qr_or_hint_url: str | None = None
    google_patents_url: str | None = None
    verification_status: str = "cnipa_result_page_parsed"
    source_origin: str = "cnipa_epub"
...
def _google_patents_url(pub_number: str | None) -> str | None:
    ...
    if not re.match(r"^(CN|US|EP|WO)\w+", pub, flags=re.IGNORECASE):
        return None
    lang = "zh" if pub.upper().startswith("CN") else "en"
    return f"https://patents.google.com/patent/{pub}/{lang}"
...
def parse_search_result_html(html: str, base_url: str = EPUB_BASE) -> list[EpubSearchHit]:
    ...
    for m in re.finditer(
        ...
        hint_url = _abs_url(href) if href else None
        pub_m = re.search(
        ...
        hits.append(
            EpubSearchHit(
                ...
                cnipa_qr_or_hint_url=hint_url,
                google_patents_url=_google_patents_url(pub_number),
            )
        )
    seen: set[str] = set()
    out: list[EpubSearchHit] = []
    for h in hits:
        key = h.pub_number or h.cnipa_qr_or_hint_url or h.title or h.raw_html[:80]
        if key in seen:
            continue
        seen.add(key)
        out.append(h)
...
def _parse_overview_card_layout(html: str) -> list[EpubSearchHit]:
    ...
    提示 URL 多在二维码 ``div.qrcode`` 的 ``title="http://epub.../patent/CN…"`` 上；
    ...
    for item_html in blocks:
        ...
        hint_url = lm.group(1).strip() if lm else None
        pm = re.search(
        ...
        if pm:
            ...
            if not re.match(r"^(?:CN|ZL)", pub_number, re.IGNORECASE):
                pub_number = None
        if not hint_url and pub_number:
            hint_url = f"{base}/patent/{pub_number}"
        if hint_url:
            m_pub = re.search(
                r"/patent/((?:CN|ZL)[^/?#]+)",
                hint_url,
        ...
        google_patents_url = _google_patents_url(pub_number)
        if not title and not pub_number and not hint_url:
            continue
        raw = "|".join(
            x for x in (title, pub_number, hint_url, (abstract or "")[:400]) if x
        )[:2000]
        hits.append(
            EpubSearchHit(
                raw_html=raw,
    ...
    out: list[EpubSearchHit] = []
    for h in hits:
        key = h.pub_number or h.cnipa_qr_or_hint_url or h.link or (h.title or "")[:120]
        if key in seen:
            continue
        seen.add(key)
        out.append(h)
...
def _parse_search_result_fallback_links(html: str) -> list[EpubSearchHit]:
    ...
    out: list[EpubSearchHit] = []
    for h in hits:
        key = h.pub_number or h.cnipa_qr_or_hint_url or h.link or h.title or ""
        if key in seen:
            continue
        seen.add(key)
        out.append(h)

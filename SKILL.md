...
description: "中国专利授权可行性审查与技术交底书生成流程（软件/算法 + 机械/结构双模式）：扫描论文、代码、方案或想法，挖掘候选专利点，按技术问题、区别特征、技术效果、实施支撑进行授权可行性初筛，联网查新并回写风险等级，生成可供代理人审稿的交底书、图示和交付检查记录。Use when users need patentability-oriented invention mining, prior-art search, disclosure drafting, iterative revision records, or attorney-review-ready patent materials."
version: "2.2.0"
user-invocable: true
...
# 专利授权可行性审查与交底书生成（软件/结构双模式）

本技能当前阶段聚焦 **专利授权可行性**：在用户提供论文、代码、方案或想法后，辅助识别具备明确技术问题、区别技术特征、可论证技术效果和实施支撑的专利方向，并生成**可供代理人审稿**的技术交底书初稿。分步指令在 **`prompts/`**，每步执行前 **`Read`** 对应文件，与步骤的对照见「Prompt 文件映射」。

本技能不替代专利代理师或律师出具正式新颖性、创造性、侵权或 FTO 法律意见；查新结论仅基于本轮可公开检索到的现有技术。

**v2.0 新增**：支持**机械/结构类专利**（装置、连接器、机构、零部件等），在 Step 1 确定专利类型后，Step 7 章节结构、附图规范、权利要求指导、Step 8 自检项均自动切换为结构模式。软件/算法类专利保持原有流程不变。
...
- **专利类型路由**：Step 1 确认后，全流程按类型切换指导。`[软件模式]` 标记的内容仅用于软件/算法类专利；`[结构模式]` 标记的内容仅用于机械/结构类专利；未标记的内容两种模式通用。
- **图示定稿（Step 7）**：
  - **[软件模式]**：**3.2**/**3.4** 用 fenced **mermaid**；执行方式、**`mmdc`** 安装与降级规则见下表「交底书定稿交付」行及 **`tools/README.md`**。
  - **[结构模式]**：**3.2** 为结构附图编号与说明（附图由用户提供 CAD/Visio 线稿或标注草图），**3.4** 为各状态/工位的结构描述与动作顺序；交底书中不强制 mermaid，可选用 mermaid 画状态转移/时序逻辑图。详见 **`references/structural_patent_requirements.md`**。
...
- **结构类专利关键词**：结构专利、机械专利、装置专利、连接器专利、零部件专利、实用新型、结构设计保护、附图标记、剖视图、装配图等
- 斜杠或简短指令：如 `/patent-unveil-review`、`/patent-disclosure`、`/交底书`
...
| **[结构模式] 附图合规参考** | **`Read`** → `${CLAUDE_SKILL_DIR}/references/structural_patent_requirements.md`，获取 CNIPA 附图规范、结构权利要求撰写要则、部件编号规范 |
...
| 参考 | `references/structural_patent_requirements.md` | **[结构模式]** CNIPA 附图规范、结构权利要求撰写要则、部件编号规范 |
...
```
□ Step 1 已确认专利类型（软件/结构），后续按对应模式执行
...
□ Step 3–4 已完成授权可行性初筛；若「区别特征清晰度」或「技术效果可论证性」为 0–1 分，未直接推荐为主方向
...
□ 除用户明确跳过外，完成摘要预览
□ [软件模式] 脱敏、mermaid（定稿均已渲染为 PNG）、章节引用符合 template_reference；含公式时 **3.4.1 符号表、§7.7 体例**（维度下标、无字母多义、LaTeX 分隔符统一）及 **3.5 符号列同形** 已满足
□ [结构模式] 已按 `disclosure_self_check.md` §8.4 执行结构专利专项自检：技术闭环完整、主实施例收敛、附图标记一致、无报告式/AI残留语言、权利要求以结构特征为主
...
□ 自检在后台完成，正文无自检清单章节；[软件模式] 含公式时已按 **`disclosure_self_check.md` §8.2** 复核**公式正确性与公式逻辑**（有误已在 Step 8 直接改稿）；[结构模式] 已按 §8.4 完成结构专项检查
```

...
将本仓库完整内容放到 Codex skills 路径下，并确保 `SKILL.md` 位于技能文件夹根级：

| 系统 | 推荐路径 |
|------|----------|
| Windows | `%USERPROFILE%\.codex\skills\patent-unveil-review\`（即 `C:\Users\<用户名>\.codex\skills\patent-unveil-review\`） |
| macOS / Linux | `~/.codex/skills/patent-unveil-review/` |

Windows（PowerShell，从 zip 安装）：
...
安装后重启 Codex 或开启新线程，再用「专利挖掘」「技术交底书」「查新」或 `patent-unveil-review` 触发。
...
或使用本地路径复制到 `.claude/skills/patent-unveil-review`。

运行时环境通常会设置 **`CLAUDE_SKILL_DIR`** 指向该技能目录；`SKILL.md` 中的 `${CLAUDE_SKILL_DIR}/prompts/...` 即解析到此路径。

## Cursor

Cursor 支持 [Agent Skills](https://www.cursor.com/docs/context/skills) 约定：每个技能是一个**子文件夹**，内含根级 `SKILL.md`（`name` 字段须与文件夹名一致，本仓库为 `patent-unveil-review`）。可将**本仓库完整内容**（含 `prompts/`、`tools/` 等）放在下列位置之一，重启 Cursor 后在 **Settings → Rules** 中查看是否已被发现；亦可用 Agent 输入 `/` 后选择技能名。

### 用户主目录（全局，所有项目可用）

| 系统 | 推荐路径 |
|------|----------|
| Windows | `%USERPROFILE%\.cursor\skills\patent-unveil-review\`（即 `C:\Users\<用户名>\.cursor\skills\patent-unveil-review\`） |
| macOS / Linux | `~/.cursor/skills/patent-unveil-review/` |

示例（将仓库克隆到全局技能目录）：

```bash
mkdir -p ~/.cursor/skills
git clone <本仓库 URL> ~/.cursor/skills/patent-unveil-review
```
...
git clone <本仓库 URL> "$env:USERPROFILE\.cursor\skills\patent-unveil-review"
```
...
`<项目根>/.cursor/skills/patent-unveil-review/`

（同样需包含完整仓库文件树，且 **`SKILL.md` 中 `name: patent-unveil-review` 与文件夹名一致**。）

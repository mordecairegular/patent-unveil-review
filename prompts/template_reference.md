...
**正文标题体例**：主流程交底书采用「一、二、三」与「（一）（二）（三）」层级。不要把 Markdown 标题写成 `## 1. ...`、`### 1.1 ...`、`### 3.4.1 ...`。流程步骤可使用 `S1、S2...`，附图可使用「图1、图2...」。
...
- **检索说明**（建议置于现有技术部分开头）：写**公开数据库名称**与**检索词**；**勿**写 `cnipa_epub_search.py` 等脚本名或「查新降级」等流程用语（见 **`prior_art_search.md`「检索说明写法」**）
- 按**技术方向**分类列举（如：单标签方法、多标签方法、聚类策略等）
- 每条现有技术正文需包含：专利号 / 文献标识、申请方（或来源机构）、技术方案、应用场景、**局限性**，并围绕本发明区别论证展开。**来源状态、稳定 URL、待复核说明在查新证据包中必填**；正式交底书正文不写 `commercial_db_content_checked`、`public_source_verified` 等状态标签。
  - **国知局 `abstract`**：若 Step 5 JSON 含 **`abstract`**，该条「技术方案」等叙述**必须先充分理解摘要后**再概括（见 **`prior_art_search.md`**）；交底书正文勿大段粘贴官方摘要全文。
  - **URL 要求**：稳定 URL、检索平台、截图编号和待复核说明写入 `prior_art_dossier.md` / 交接说明；正式正文如确需列出公开来源，优先只写公开文献号、题名和公开日，不写内部核验状态。
  - **正文呈现建议**：表格列优先使用「公开文献 / 技术方向 / 与本发明的接近点 / 与本发明的主要区别」；不要增列“来源状态/来源链接/待复核”。
...
- 流程涉及算法、评分、约束或形式化变量时，在「符号与公式」小节集中给出符号定义与主公式；须遵守 **`disclosure_builder.md` §7.7**
...
- 置信度/阈值类：含义、取值范围
- 算法参数：公式、约束条件
- 参数表须设 **「符号」列**，与「符号与公式」小节的符号表逐字同形（勿在参数表改用 `^{cpu}` 而上文用下标）
- 确保与正文公式、实施例数值一致
...
- 列出核心创新点，每点简明定义
- 详细技术方案引用第三章对应小节（如「具体实现见第三章‘符号与公式’」）
- 避免与第三章重复大段技术细节
